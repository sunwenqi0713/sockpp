...
Socket::Status TcpSocket::connect(IpAddress remoteAddress, unsigned short remotePort,
  ...
  Status status = detail::SocketImpl::getErrorStatus();
  ...
  if (status == Socket::Status::NotReady) {
    ...
    if (select(static_cast<int>(getNativeHandle() + 1), nullptr, &selector, nullptr, &time) > 0) {
      ...
      // To know whether it's a success or a failure, we must check the address
      ...
      if (getRemoteAddress().has_value()) {
        ...
        // Connection refused.
        status = detail::SocketImpl::getErrorStatus();
      }
    } else {
...
#pragma GCC diagnostic pop
    ...
    // Check for errors.
    if (result < 0) {
      const Status status = detail::SocketImpl::getErrorStatus();
...
Socket::Status TcpSocket::receive(Packet& packet) {
  ...
  if (m_pendingPacket.sizeReceived < sizeof(m_pendingPacket.size)) {
    ...
    packet_size = ntohl(m_pendingPacket.size);
  } else {
  ...
  while (m_pendingPacket.data.size() < packet_size) {
    // Receive a chunk of data.
    const std::size_t size_to_get = std::min(packet_size - m_pendingPacket.data.size(), buffer.size());
    const Status status = receive(buffer.data(), size_to_get, received);
    if (status != Status::Done) {
      return status;
    }
